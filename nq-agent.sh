...
# Get network latency
ping_eu=$(prep $(num "$(ping -c 2 -w 2 fra.mandar.me | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_us=$(prep $(num "$(ping -c 2 -w 2 nyc.mandar.me | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_as=$(prep $(num "$(ping -c 2 -w 2 nyc.mandar.me | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
...
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/nodequery/nq-agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.mandar.me/webhook/5a960b45-233a-4e01-a0d6-14acf3924e07"
else
	wget -q -o /dev/null -O /etc/nodequery/nq-agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.mandar.me/webhook/5a960b45-233a-4e01-a0d6-14acf3924e07"
	wget_pid=$! 
	wget_counter=0
	wget_timeout=30
